...
import { getProjects, addProject, updateProject, deleteProject, getTasks, addTask, updateTask, deleteTask, getEmployees } from './firebase';

export default function ProjectsModule({ currentTenant, currentUser, customers = [] }) {
  ...
  const [editingProjectData, setEditingProjectData] = useState({ name: '', description: '', clientId: '', clientName: '', startDate: '', endDate: '', status: 'Active' });
  ...
  const handleCreateProject = () => {
    setEditingProjectData({ name: '', description: '', clientId: '', clientName: '', startDate: '', endDate: '', status: 'Active' });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Project Tracker</h1>
          <p className="text-sm text-slate-500">Track tasks, manage team projects, and review performance</p>
        </div>
...
<button onClick={handleCreateProject} className="bg-recloud-600 hover:bg-recloud-700 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-md shadow-recloud-500/20 flex items-center gap-2 transition-all">
      ...
      {!activeProject ? (
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center p-8">
          ...
          <p className="text-slate-500 mb-6 max-w-md text-center">Create a new project to start tracking tasks and collaborating with your team.</p>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              ...
              <button onClick={saveProject} className="px-5 py-2.5 bg-blue-600 text-white font-bold hover:bg-blue-700 hover:shadow-md rounded-xl transition-all">Save Project</button>
            </div>
          </div>
        </div>
      )}
