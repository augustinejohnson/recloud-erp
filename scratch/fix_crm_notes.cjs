...
const searchStr = `                <button onClick={() => { setIsCustomerDetailsOpen(false); setIsEmailModalOpen(true); }} className="bg-white border border-slate-200 hover:border-blue-300 hover:shadow-md text-slate-700 p-3 rounded-xl flex flex-col items-center gap-2 transition-all group">
...
const replaceStr = `                <button onClick={() => { setIsCustomerDetailsOpen(false); setIsEmailModalOpen(true); }} className="bg-white border border-slate-200 hover:border-blue-300 hover:shadow-md text-slate-700 p-3 rounded-xl flex flex-col items-center gap-2 transition-all group">
                  <div className="w-10 h-10 rounded-full bg-blue-50 text-blue-500 flex items-center justify-center group-hover:bg-blue-500 group-hover:text-white transition-colors">
                    <Mail className="w-5 h-5" />
                  </div>
                  <span className="text-xs font-bold">Send Email</span>
...
if (content.includes(searchStr)) {
  ...
  console.log('Failed to find replacement string. Dumping current file content snippet around Button...');
  const idx = content.indexOf('Send Email</span>');
